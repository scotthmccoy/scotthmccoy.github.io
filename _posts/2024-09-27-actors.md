...
- Move "what thread am I on" issues from a runtime to compile-time. For example, you never need to check `Thread.isMainThread` anymore, you can just mark a func as `@MainActor`:
![image](https://github.com/user-attachments/assets/f95a7b08-b17f-423e-9606-a1b0b61e04b5)
- Given that it's reentrant, it seems to have chosen the evils of race conditions over deadlock; a weirdly behaving app is better than a completely frozen one.
...
This might be frustrating, but implementing a consistent isolation pattern requires choosing between the potential for either race conditions or deadlock. Swift chose race conditions as their evil and we have to live with it. Thankfully, the solution is pretty easy: implement a Task pool:
